...
func newFileSet(env *env, p string, r *FileSet) (*fileSet, error) {
	...
	for _, sel := range r.Select {
		...
		if strings.HasSuffix(sel, "/**") || sel == "**" {
			...
			if err != nil {
				return nil, fmt.Errorf("list all files %q: %w", sel, err)
			}
			matches = files
		} else {
			glob, err := filepath.Glob(env.src(makeRelPath(p, sel)))
			if err != nil {
				return nil, fmt.Errorf("glob %q: %w", sel, err)
			}
			matches = glob
		}
		if len(matches) == 0 {
...
func (fs *fileSet) build(env *env, opts *buildOpts) error {
	...
	for _, f := range fs.files {
		...
		case nodeSrc:
			...
			if err != nil {
				return fmt.Errorf("file stat %q: %w", f, err)
			}
			add(s)
		case nodeOut:
			s, err := newOutFileStat(env, f)
			if err != nil {
				return fmt.Errorf("out file stat %q: %w", f, err)
			}
			add(s)
		default:
	...
	for _, inc := range fs.includes {
		...
		var list []*fileStat
		if err := readJSONFile(env.out(fileSet), &list); err != nil {
			return fmt.Errorf("read file set %q: %w", inc, err)
		}
	...
	if err := writeJSONFile(out, list); err != nil {
